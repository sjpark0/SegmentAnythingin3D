...
class SCView_Dataset(Dataset):
    ...
    def load_meta(self):
        ...
        poses_arr = np.load(os.path.join(self.root_dir, "poses_bounds1.npy"))
        ...
        self.image_paths, self.image_poses, self.image_times, N_cam, N_time = self.load_images_path1(self.root_dir, self.split)
    ...
    def load_images_path1(self, datadir, split):
        image_paths=[]
        image_poses=[]
        image_times=[]
        N_cams = 0
        ...
        folder_list = [folder for folder in sorted(folder_list) if folder.startswith("cam") and not folder.endswith(".mp4")]
        for index, folder in enumerate(folder_list):
            ...
            N_cams +=1
            count = 0
            image_path = os.path.join(datadir, folder, "images")
            images_path = os.listdir(image_path)
            images_path.sort()
            this_count = 0
            image_length = len(images_path)
            for idx, path in enumerate(images_path):
                ...
                R[:,0] = -R[:,0]
                T = -pose[:3,3].dot(R)
                image_times.append(idx/image_length)
